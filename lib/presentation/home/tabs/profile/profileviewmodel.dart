...
class Profileviewmodel extends Cubit<ProfileStates> {
  ...
  final String token = SecureStorageService().getToken() as String;
  ...
  List<user_LoggedUserDataResponse_entity> userDataList = [];
  ...
  void getloggeduserdata() async {
    ...
    switch (result) {
      case Success():
        ...
        emit(state.copyWith(status:ProfileStatus.success,obj_user_LoggedUserDataResponse_entity: userDataList??[]));
...
class loggeduserdata extends profileIntent {
  final String token;
  loggeduserdata({required this.token});
}
